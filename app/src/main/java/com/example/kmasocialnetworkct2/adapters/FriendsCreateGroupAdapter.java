...
public class FriendsCreateGroupAdapter extends RecyclerView.Adapter<FriendsCreateGroupAdapter.ViewHolderFriendsCreateGroup>{
    ...
    public FriendsCreateGroupAdapter(ArrayList<Friends> list, Context context, String nameGroup, String idGroup) {
        this.list = list;
    ...
    @NotNull
    @Override
    public FriendsCreateGroupAdapter.ViewHolderFriendsCreateGroup onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.sample_show_list_friend_create_group, parent, false);
        return new ViewHolderFriendsCreateGroup(view);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull FriendsCreateGroupAdapter.ViewHolderFriendsCreateGroup holder, int position) {
...
//                database.getReference().child("UserGroups").child(participant.getUserId()).child(groupId).setValue();
                ...
                Toast.makeText(context, "Thêm thành công!!! "+ friend.getFriendName(), Toast.LENGTH_SHORT).show();

                database.getReference().child("GroupsPost").child(idGroup).child("groupName").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                    @Override
                    public void onComplete(Task<DataSnapshot> task) {
                        ...
                        database.getReference().child("UserGroupsPost").child(participant.getUserId()).child(idGroup).setValue(userGroup);
                    }
                });
    ...
    public class ViewHolderFriendsCreateGroup extends RecyclerView.ViewHolder {

        SampleShowListFriendCreateGroupBinding binding;

        public ViewHolderFriendsCreateGroup(@NonNull @NotNull View itemView) {
            super(itemView);
